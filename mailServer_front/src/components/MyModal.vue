<template>
  <button  @click="open = true, clearFormName()" class="side-btn">Create Folder</button>
  <Teleport to="body" >
    <transition name="fade">
      <div v-if="open" class="modal" @mousedown="startDrag" v-bind:style="{ top: top + 'px', left: left + 'px' }">
        <div class="p">New Folder</div>
        <div class="formElement">
        <input type="text" v-model="folderName.value" 
        id="folder name" placeholder="Enter folder name"
        @keypress="k => { if (k.key == 'Enter') handleFormName() }" />
        </div>
        <button class="ocbtn" @click="open = false">Close</button>
        <button class="ocbtn" @click="handleFormName(), open = false">Create</button>
      </div>
  ...
  props:{
    ...
    methods: {
      ...
      sendFolder() {
        ...
        //this.$emit("sendFolder", [ ...this.folders ]);
